...
exports.getAll = async (req, res) => {
    try {
        ...
        const results=await Product.find(filter).sort(sort).populate("brand").select('-images').skip(skip).limit(limit).exec()
...
exports.getImagesById=async(req,res)=>{
    try {
        ...
        // Generate images based on product title using Picsum API
        const images = [
            `https://picsum.photos/seed/${encodeURIComponent(product.title)}/640/480`,
            `https://picsum.photos/seed/${encodeURIComponent(product.title)}1/640/480`,
            `https://picsum.photos/seed/${encodeURIComponent(product.title)}2/640/480`,
            `https://picsum.photos/seed/${encodeURIComponent(product.title)}3/640/480`,
            `https://picsum.photos/seed/${encodeURIComponent(product.title)}4/640/480`,
        ]
        res.status(200).json(images)
    } catch (error) {
        console.log(error);
        res.status(500).json({message:'Error fetching product images, please try again later'})
    }
}
