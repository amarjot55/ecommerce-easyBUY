...
import { addProduct, deleteProductById, fetchProductById, fetchProductImagesById, fetchProducts, undeleteProductById, updateProductById } from "./ProductApi";
...
export const fetchProductImagesByIdAsync=createAsyncThunk("products/fetchProductImagesByIdAsync",async(id)=>{
    const images=await fetchProductImagesById(id)
    return images
})
...
const productSlice=createSlice({
    ...
    reducers:{
        ...
        resetProductImagesFetchStatus:(state)=>{
            state.productImagesFetchStatus='idle'
        },
        clearSelectedProductImages:(state)=>{
    ...
    extraReducers:(builder)=>{
        builder
            ...
            .addCase(fetchProductImagesByIdAsync.pending,(state)=>{
                state.productImagesFetchStatus='pending'
            })
            .addCase(fetchProductImagesByIdAsync.fulfilled,(state,action)=>{
                state.productImagesFetchStatus='fullfilled'
                state.selectedProductImages[action.meta.arg]=action.payload
            })
            .addCase(fetchProductImagesByIdAsync.rejected,(state,action)=>{
                state.productImagesFetchStatus='rejected'
                state.errors=action.error
            })
...
export const selectProductImagesFetchStatus=(state)=>state.ProductSlice.productImagesFetchStatus
export const selectSelectedProductImages=(state)=>state.ProductSlice.selectedProductImages

// exporting actions
export const {clearProductSuccessMessage,clearProductErrors,clearSelectedProduct,resetProductStatus,resetProductUpdateStatus,resetProductAddStatus,toggleFilters,resetProductFetchStatus,resetProductImagesFetchStatus,clearSelectedProductImages}=productSlice.actions
