...
import { fetchProductImagesByIdAsync, selectSelectedProductImages } from '../ProductSlice';
...
export const ProductCard = ({id,title,price,thumbnail,brand,stockQuantity,handleAddRemoveFromWishlist,isWishlistCard,isAdminCard}) => {
    ...
    const selectedProductImages=useSelector(selectSelectedProductImages)
    const dispatch=useDispatch()
    let isProductAlreadyinWishlist=-1

    useEffect(()=>{
        if(id && !selectedProductImages[id]){
            dispatch(fetchProductImagesByIdAsync(id))
        }
    },[id, dispatch, selectedProductImages])
  ...
  return (
    ...
    <Stack component={isAdminCard?"":isWishlistCard?"":is408?'':Paper} mt={is408?2:0} elevation={1} p={2} width={is408?'auto':is488?"200px":is608?"240px":is752?"300px":is932?'240px':is1410?'300px':'340px'} sx={{cursor:"pointer"}} onClick={()=>navigate(`/product-details/${id}`)}>
        ...
        {/* image display */}
        <Stack>
            <img width={'100%'} style={{aspectRatio:1/1,objectFit:"contain"}} height={'100%'}  src={selectedProductImages && selectedProductImages[id] && selectedProductImages[id].length > 0 ? selectedProductImages[id][0] : thumbnail} alt={`${title} photo unavailable`} />
        </Stack>
